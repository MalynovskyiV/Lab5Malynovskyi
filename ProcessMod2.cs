...
{
    internal static class ProcessMod2
    {
        private static readonly Thread UpdateDbThread;
        private static readonly Thread UpdateEntriesThread;

        internal static Dictionary<int, ProcessMod1> Processes { get; set; }

        static ProcessMod2()
        {
            Processes = new Dictionary<int, ProcessMod1>();
                    ...
                    lock (Processes)
                    {
                        ...
                        foreach (System.Diagnostics.Process proc in processes)
                        {
                            if (!Processes.ContainsKey(proc.Id))
                            {
                                try
                                {
                                    Processes[proc.Id] = new ProcessMod1(proc);
                                }
